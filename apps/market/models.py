...
class CoinExchangeOffer(models.Model):
    coin_origin = models.ForeignKey(Coin, on_delete=models.CASCADE)
    coin_provider = models.ForeignKey(CoinProvider, on_delete=models.CASCADE)
    coin_exchange = models.ForeignKey(Coin, on_delete=models.CASCADE)
...
class Operation(models.Model):
    ...
    tax = models.DecimalField(_("available amount"), validators=[MinValueValidator(0)], decimal_places=2)


class MarketConfig(models.Model):
    fee = models.DecimalField(_("fee in %"), validators=[MinValueValidator(0)], decimal_places=2)
